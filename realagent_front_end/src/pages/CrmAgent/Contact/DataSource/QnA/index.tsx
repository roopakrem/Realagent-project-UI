...
import { getAllQuestionAnswers } from "../../../../../store/features/DataSources";
...
const QnADataSource: React.FC = () => {
  ...
  const [isEditingLoading, startEditingLoading, finishEditingLoading] =
    useLoadingState();
  const [questionAnswer, setQuestionAnswer] =
    useState<CreateQuestionAnswerFormData>({ question: "", answer: "" });
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editQuestionAnswer, setEditQuestionAnswer] =
    useState<CreateQuestionAnswerFormData>({ question: "", answer: "" });
  const allQuestionAnswers = useAppSelector(
    (state) => state.dataSources.allQuestionAnswers
  );
  ...
  const onSaveEdit = async (id: string) => {
    startEditingLoading();
    try {
      const result = await questionAnswerAPI.updateQuestionAnswer(
        id,
        editQuestionAnswer
      );
      if (result?.status === ApiResponseStatus.SUCCESS) {
          ...
          {allQuestionAnswers.map((qna) => (
            ...
            >
                ...
                {editingId === qna._id ? (
                  ...
                  <>
                    <EditButton
                      ...
                      onClick={() =>
                        onQuestionAnswerEdit(qna._id, qna.question, qna.answer)
                      }
                    />
                    <WithModal
              ...
              <Textarea
                ...
                value={
                  editingId === qna._id ? editQuestionAnswer.answer : qna.answer
                }
                onChange={(e) =>
                  setEditQuestionAnswer({
                    ...editQuestionAnswer,
