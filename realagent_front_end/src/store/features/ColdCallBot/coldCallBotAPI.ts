...
export type Script = {
  introMessagePrompt: string;
  collectInfoPrompt: string;
  exitFlowPrompt: string;
...
export type UpdateTwilioConfigurationFormdata = {
  forwardingNumber?: string;
  agentType: AgentType;
};
...
const getAvailableNumber = () => {
  return VAPI_API.get<NumbersList>({
    ...
    queryParameters: {
      agentType: AgentType.ColdCalling,
    },
  });
};
...
const getSavedNumber = (agentType: AgentType) => {
  return VAPI_API.get<SavedNumber>({
    slug: Slug.GETSAVEDNUMBER,
    queryParameters: { agentType },
  });
};
const getVoice = () => {
...
const getVoiceConfig = (agentType: AgentType) => {
...
const AddVoice = (formData: AddVoiceFormdata) =>
  VAPI_API.post<AddedVoiceList>({
    ...
    body: { ...formData, agentType: AgentType.ColdCalling },
  });
...
const updateTwilioConfiguration = (formData: UpdateTwilioConfigurationFormdata) => {
  return VAPI_API.put<AddedVoiceList>({
    slug: Slug.TWILLIOCONFIG,
    body: formData,
  });
};
