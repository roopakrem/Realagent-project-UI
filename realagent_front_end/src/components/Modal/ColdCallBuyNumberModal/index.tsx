...
const ColdCallBuyNumberModal: React.FC<BuyNumberModalProps> = ({ open, onClose }) => {
  const dispatch = useAppDispatch();
  const availableNumber = useAppSelector((state) => state.coldCallBot?.number?.phoneNumber);
  ...
  const handleSubmit = async () => {
    ...
    if (availableNumber) {
      ...
      try {
        await dispatch(
          addTwilioNumber({ phoneNumber: availableNumber, organizationName, agentType: AgentType.ColdCalling }),
        ).unwrap();
...
export default ColdCallBuyNumberModal;
